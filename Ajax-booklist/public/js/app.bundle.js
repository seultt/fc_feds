  ...
  _createClass(BookList, [{
    ...
    value: function bindEvent() {
      ...
      document.querySelector('#btn-add').addEventListener('click', function (e) {
        ...
        _this2.books.push({
          id: !_this2.books.length ? 1 : Math.max.apply(Math, _toConsumableArray(_this2.books.map(function (_ref2) {
            var id = _ref2.id;
            return id;
          }))) + 1, // id가 제일 큰 값을 가져와 +1 시킨다.
          title: '',
          author: '',
          price: '',
          editable: true,
      ...
      document.querySelector('tbody').addEventListener('click', function (e) {
        ...
        switch (type) {
          ...
          case 'edit':
            {
              _this2.books.forEach(function (item) {
                if (item.id === targetId) {
                  item.editable = true;
                  item.status = "edited";
                }
          ...
          case 'save':
            {
              ...
              if (!title) {
                console.log('??');return document.querySelector('#title').setAttribute('placeholder', 'title을 입력하세요');
              }

              _this2.books.forEach(function (item) {
                if (item.id === targetId) {

                  var addData = { id: Math.max.apply(Math, _toConsumableArray(_this2.books.map(function (_ref3) {
                      var id = _ref3.id;
                      return id;
                    }))), title: title, author: author, price: price, editable: false, status: '' };
                  var editData = { id: item.id, title: title, author: author, price: price, editable: false, status: '' };

                  if (item.status === 'new') {
                    console.log('new');

                    _ajax2.default.post(_this2.url, addData).then(function (data) {
                      _this2.books = JSON.parse(data);
                      console.log(_this2.books);
                      _this2.bindBooksToDom();
                    });
                  } else if (item.status === 'edited') {
                    console.log('edited');
                    _ajax2.default.put(_this2.url, item.id, editData).then(function (data) {
                      console.log(data);
                      _this2.books = JSON.parse(data);
                      _this2.bindBooksToDom();
                    });
          ...
          case 'delete':
            {
              var _targetId = parseInt(e.target.dataset.item);

              _ajax2.default.delete(_this2.url, _targetId).then(function (data) {
                _this2.books = JSON.parse(data);
                console.log(_this2.books);
                _this2.bindBooksToDom();
              });
          ...
          case 'cancel':
            {
              _ajax2.default.get(_this2.url).then(function (data) {
                // console.log(data, typeof data);
                _this2.books = JSON.parse(data);
                _this2.bindBooksToDom();
              });
